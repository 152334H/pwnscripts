...
class ROP(rop.rop.ROP):
	...
	def system_call(self, num:int, args:list):
		...
		0x0040:   b'/bin/sh\x00'
		'''
		if context.arch != 'amd64': raise NotImplementedError("syscall_call() is only implemented for amd64 right now")
		if self.syscall is None: raise AttributeError("ROP unable to find syscall gadget")
		self.pop({'rax': num})
