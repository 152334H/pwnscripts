...
class ROP(rop.rop.ROP):
    ...
    def system_call(self, id: Union[int,str], args: list, ret: bool=False) -> None:
        ...
        if ret:
            ...
            syscall = self.find_gadget([rop.srop.syscall_instructions[context.arch][0], 'ret'])
        else:   # Can lazily use ROP's __getattr__ here
            syscall = self.syscall
        if syscall is None:
            raise AttributeError("ROP unable to find syscall gadget")

        # get label for Call(); convert id to numerical register value
        if type(id) == int:
        ...
        # write the syscall
        self.raw(Call(label, syscall.address, [id] + args, ABI.syscall()))